...
The code in this repository generates all of the figures for [Dixon et al 2025](https://doi.org/10.1016/j.cels.2025.101265). It is primarily intended to support reproducibility of our research. In addition, researchers may find parts of this code valuable for future work.

The `dev` branch reflects the most up to date version of this analysis. To exactly reproduce the analysis as seen in the [bioRxiv](https://www.biorxiv.org/content/10.1101/2024.06.28.601071v1) version of the manuscript use the `bioRxiv-v1` branch.
...
## Installation
This analysis is tested on Ubuntu 18.04. Other operating systems may work.

1. Install `git` and [uv](https://docs.astral.sh/uv/getting-started/installation/). Run `uv --version` to validate the installation.
...
List available workflows with the following command. The first time you run one of these commands, expect some extra start-up time (up to a couple minutes) as python dependencies get cached.
```bash
uv run python run_all_manuscript_workflows.py --list
```

Use the `--only` flag to run any one workflow. Confirm that your installation is working by running a fast workflow.
```bash
uv run python run_all_manuscript_workflows.py --only error
```
...
```bash
uv run python run_all_manuscript_workflows.py
```
...
```bash
uv run python nuc_morph_analysis/lib/preprocessing/save_datasets_for_quilt.py
```
...
```bash
uv run python nuc_morph_analysis/lib/preprocessing/generate_main_manifest.py
```
...
```bash
uv run python nuc_morph_analysis/lib/preprocessing/generate_perturbation_manifest.py
```
...
```bash
uv run python nuc_morph_analysis/lib/visualization/write_data_for_colorizer.py
```
